...
2026-03-23 の review 時点では、source build の Windows headless は確認できていますが、CI で組み立てる release package は editor/headless 向け debug binary と一部 runtime dependency の取り込みが未解決です。source checkout 直後は全 platform の binary も揃っていないため、現状は source build または package 内容の個別確認を前提に扱ってください。
...
- ただし、2026-03-23 時点の package review では Windows editor/headless 向け load 失敗を再現しており、release package の導線は引き続き是正対象です。
