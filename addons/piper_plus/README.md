...
一方で macOS packaged smoke の CI 実行結果、Android/iOS の Godot export / runtime 検証は継続中です。source checkout 直後は全 platform の binary が揃っていないため、local では source build または package 内容の個別確認を前提に扱ってください。
...
- Windows packaged addon smoke は local で再確認済みです。macOS packaged smoke と mobile export/runtime は継続中です。
