...
godot-piper-plusは、[piper-plus](https://github.com/ayutaz/piper-plus)（VITS ニューラルTTSエンジン）をGodotエンジン向けにGDExtensionとして移植するプロジェクト。Windows、Android、iOS、Linux、macOS を CI 対象にしている。

Unity向け移植 [uPiper](https://github.com/ayutaz/uPiper) を参考実装として扱う。uPiper v1.2.0でネイティブOpenJTalkは完全削除され、dot-net-g2p（純C#）に移行済み。

### 現状注記

- 現在の runtime で text input 合成が動くのは日本語 OpenJTalk 経路。
- `custom_dictionary_path` によるカスタム辞書は runtime 接続済み。
- 英語 G2P、`language_id`、multilingual phonemizer は未実装で、この文書内では「目標構成」と「現状実装」を区別して読むこと。

### 参照プロジェクト

| プロジェクト | パス / URL | 役割 |
|-------------|-----------|------|
| piper-plus | `../piper-plus` / https://github.com/ayutaz/piper-plus | TTSエンジン本体（C++/Python） |
| uPiper | https://github.com/ayutaz/uPiper | Unity向け移植（参考実装） |
