...
- `pull_request` では `build-pages-demo` だけを実行して Pages demo の build と local smoke を先に検証する
- deploy は `main` push の自動実行に加えて、`workflow_dispatch` で `deploy_pages=true` のときだけ current ref に対して試行する
