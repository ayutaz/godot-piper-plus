...
| **M4** | CI/CD（全プラットフォーム） | M1 | GitHub Actionsで5プラットフォームの自動ビルド | 未着手 |
| **M5** | モデル/辞書管理 + ドキュメント | M2 | XMLドキュメント表示、モデルダウンロード機能 | 未着手 |
| **M6** | ストリーミング合成 | M3 | AudioStreamGeneratorで逐次再生 | 未着手 |
