...
| 1.6 | piper-phonemize ExternalProject | `cmake/PiperPhonemize.cmake` | espeak-ng含む、静的リンク |
...
|--------------------------|--------|--------|
| `ExternalProject_Add(hts_engine_api ...)` | `cmake/HTSEngine.cmake` | そのまま流用可能 |
| `ExternalProject_Add(open_jtalk ...)` | `cmake/OpenJTalk.cmake` | そのまま流用可能 |
| `ExternalProject_Add(piper_phonemize ...)` | `cmake/PiperPhonemize.cmake` | 静的リンクに変更 |
| ONNX Runtime検出ロジック | `cmake/FindOnnxRuntime.cmake` | find_packageパターンに整理 |
| MSVC `/utf-8`, `/MD` 設定 | `CMakeLists.txt` | そのまま流用 |

### 完了基準

- [ ] `cmake -B build && cmake --build build` がエラーなく完了する
- [ ] OpenJTalk, HTSEngine, piper-phonemize が静的リンクされている
- [ ] ONNX Runtime が動的リンクされている
...
|-------------------|------|-------------------|
| `piper.cpp` | 1,840 | spdlog → Godot `UtilityFunctions::print` に置換 |
| `piper.hpp` | 186 | そのまま |
| `openjtalk_phonemize.cpp/hpp` | 244 | そのまま |
| `custom_dictionary.cpp/hpp` | 536 | そのまま |
| `phoneme_parser.cpp/hpp` | 219 | そのまま |
...
| ファイル | 推定行数 | 内容 |
|---------|---------|------|
| `src/piper_tts.h` | 80-100 | PiperTTSクラス定義（Node継承） |
| `src/piper_tts.cpp` | 300-400 | 初期化、同期合成、プロパティバインド |
| `src/register_types.cpp` | 30-50 | ClassDB登録 |
...
- [ ] `PiperTTS` ノードがGodotエディタのノード追加ダイアログに表示される
- [ ] Inspectorでmodel_path等のプロパティが設定可能
- [ ] GDScriptから `tts.initialize()` → `tts.synthesize("こんにちは")` で `AudioStreamWAV` が返る
- [ ] 返された `AudioStreamWAV` を `AudioStreamPlayer` で再生し、音声が聞こえる
- [ ] 日本語・英語モデルの両方で動作確認
...
| piper.cpp の spdlog 依存除去の複雑さ | M2 遅延 | spdlogも静的リンクで残す案をフォールバックに |
