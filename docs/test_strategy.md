...
- Layer 1 は実装済みで、`ctest --test-dir build-test --output-on-failure` で **109テスト** を通している
- Layer 1 の CI は `.github/workflows/build.yml` で desktop 3プラットフォームに対して `Configure with tests` → `Build tests` → `Run C++ unit tests` を実行する
- Layer 2 は `test/` 以下に Godot headless 実行基盤を実装済みで、`test/run-tests.sh` と `test/prepare-assets.sh` により GDExtension とテストモデルを `test/project/` に配置して走らせる
- Layer 2 は現時点ではローカル/手動実行前提で、GitHub Actions workflow にはまだ組み込んでいない
...
─────────────────────────────────────────────────────────────────────────
Layer 1: C++ ユニットテスト   Google Test 1.14     CTest              desktop 3プラットフォーム
Layer 2: GDScript 統合テスト  godot-cpp test方式   Godot --headless   ローカル基盤あり（workflow未接続）
...
| 層 | テスト数目安 | 実行時間 | 目的 |
|----|------------|---------|------|
| Layer 1 | 100件前後 | < 30秒 | 個々の関数・クラスの正確性 |
...
実装では `src/piper_core/piper_test_utils.cpp` をテスト用の共通ヘルパーとして追加し、`parse*Config`、`scaleAudioToInt16`、`createWavHeader` などを本番コードと共有している。
...
| ONNX Runtime | **条件付きテスト**（`res://models` または `PIPER_TEST_*` 環境変数） | モデルファイル依存のテストはCIでスキップ可 |
...
├── prepare-assets.sh           # addon/bin とテストモデルを test/project/ に配置
├── project/
...
│   ├── models/                 # bundled test model 配置先
│   └── addons/piper_plus/
│       ├── piper_plus.gdextension
│       └── bin/                # ビルド済バイナリ配置先（.gitignoreで管理）
```
...
| `test_synthesize_async_without_init` | 未初期化で synthesize_async() が `ERR_UNCONFIGURED` を返す | 不要 |
...
func _has_test_model() -> bool:
    return FileAccess.file_exists(BUNDLED_MODEL_PATH) \
        or not OS.get_environment("PIPER_TEST_MODEL_PATH").is_empty()
...
|----------------|-------------------|-------------------|------|
| Windows x86_64 | o | △ | デスクトップ、Godot headless 可能。Layer 2 workflow 未接続 |
| Linux x86_64 | o | △ | デスクトップ、Godot headless 可能。Layer 2 workflow 未接続 |
| macOS arm64 | o | △ | デスクトップ、Godot headless 可能。Layer 2 workflow 未接続 |
...
| Phase 3 | GDScript 統合テスト（Layer 2）を workflow に接続 | GDExtension API 検証 |
